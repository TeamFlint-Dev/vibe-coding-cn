# ⬆️ 升级系统 - 机制设计

> **系统**：Upgrade System  
> **版本**：v1.0  
> **前置依赖**：`@mechanics/income.md`  
> **生成者**：game-mechanics-designer

---

## 一、系统概述

**核心职责**：管理玩家的能力提升和进度解锁。

**设计目标**：
- 提供明确的成长感和目标
- 升级效果立竿见影
- 费用曲线合理，不卡进度也不过快

---

## 二、MVP 升级项

### 2.1 鱼竿等级

**唯一升级项**（MVP 阶段聚焦）

| 属性 | 描述 |
|------|------|
| 名称 | 鱼竿等级 |
| 最大等级 | 10 |
| 效果 | 提升稀有鱼掉落概率 |
| 解锁条件 | 无（初始可用） |

### 2.2 等级效果表

| 等级 | 稀有加成 | 累计效果 | 升级费用 | 累计投入 |
|------|----------|----------|----------|----------|
| 1 | - | 基础 | - | 0 |
| 2 | +5% | +5% | 100 | 100 |
| 3 | +5% | +10% | 150 | 250 |
| 4 | +5% | +15% | 225 | 475 |
| 5 | +5% | +20% | 337 | 812 |
| 6 | +5% | +25% | 506 | 1,318 |
| 7 | +5% | +30% | 759 | 2,077 |
| 8 | +5% | +35% | 1,139 | 3,216 |
| 9 | +5% | +40% | 1,708 | 4,924 |
| 10 | +5% | +45% | 2,562 | 7,486 |

**费用公式**：
```
UpgradeCost(level) = Floor(100 × 1.5^(level-2))
```

### 2.3 稀有度加成机制

```
原始掉落权重:
  Common:   550 (69.6%)
  Uncommon: 150 (19.0%)
  Rare:     60  (7.6%)
  Epic:     30  (3.8%)

应用 +20% 稀有加成后 (Lv5):
  # Rare 及以上权重增加
  Rare:     60 × 1.2 = 72
  Epic:     30 × 1.2 = 36
  
  # Common 权重减少以补偿
  减少量 = (72-60) + (36-30) = 18
  Common: 550 - 18 = 532
  
调整后概率:
  Common:   532 (67.3%)  ↓
  Uncommon: 150 (19.0%)  -
  Rare:     72  (9.1%)   ↑
  Epic:     36  (4.6%)   ↑
```

---

## 三、升级流程

### 3.1 状态机

```
              ┌────────────┐
              │   LOCKED   │  (等级不可用)
              └────────────┘
                    │
                    │ [满足前置条件]
                    ▼
              ┌────────────┐
              │ AVAILABLE  │  (可升级)
              └────────────┘
                    │
            ┌───────┴───────┐
            │               │
      [金币不足]       [金币足够]
            │               │
            ▼               ▼
      ┌────────────┐  ┌────────────┐
      │ AFFORDABLE │  │  UPGRADE   │
      │   = false  │  │  CONFIRM   │
      └────────────┘  └────────────┘
                            │
                            │ [确认升级]
                            ▼
                      ┌────────────┐
                      │  UPGRADED  │
                      └────────────┘
                            │
                            ▼
                      ┌────────────┐
                      │  下一级 ?  │
                      └────────────┘
```

### 3.2 升级流程

```
┌──────────────┐
│ 玩家打开商店 │
└──────────────┘
       │
       ▼
┌──────────────┐
│ 显示升级选项 │
│ 当前等级     │
│ 下一级效果   │
│ 升级费用     │
└──────────────┘
       │
       │ 点击「升级」
       ▼
┌──────────────┐
│ 验证金币     │
└──────────────┘
       │
  ┌────┴────┐
  │         │
不足      足够
  │         │
  ▼         ▼
提示     ┌──────────────┐
金币     │ 扣除金币     │
不足     │ 等级 +1      │
         │ 应用效果     │
         │ 播放动画     │
         └──────────────┘
              │
              ▼
         ┌──────────────┐
         │ 通知钓鱼系统 │
         │ 更新掉落表   │
         └──────────────┘
```

---

## 四、数据结构

```verse
upgrade_system := class:
    var RodLevel : int = 1              # 当前鱼竿等级
    
    # 常量
    MAX_ROD_LEVEL : int = 10
    BASE_COST : int = 100
    COST_MULTIPLIER : float = 1.5
    RARITY_BONUS_PER_LEVEL : float = 0.05

upgrade_item := struct:
    ItemId : string           # "rod_upgrade"
    CurrentLevel : int
    MaxLevel : int
    UpgradeCost : int
    EffectDescription : string
```

---

## 五、事件接口

### 5.1 接收事件

| 事件 | 来源 | 处理 |
|------|------|------|
| `OnGoldChanged` | 收益系统 | 更新可购买状态 |

### 5.2 发出事件

| 事件 | 触发条件 | 数据 |
|------|----------|------|
| `OnUpgradeCompleted` | 升级成功 | itemId, newLevel |
| `OnRarityBonusChanged` | 鱼竿升级 | newBonusPercent |

---

## 六、UI 设计

### 6.1 升级商店界面

```
┌────────────────────────────────────────────────┐
│  🔧 升级商店                        [X] 关闭   │
├────────────────────────────────────────────────┤
│                                                │
│  ┌──────────────────────────────────────────┐  │
│  │  🎣 鱼竿                                 │  │
│  │                                          │  │
│  │  当前等级: ⭐⭐⭐⭐⭐ (Lv.5)              │  │
│  │  当前效果: 稀有掉落 +20%                 │  │
│  │                                          │  │
│  │  ─────────────────────────────────────   │  │
│  │                                          │  │
│  │  下一等级: Lv.6                          │  │
│  │  升级效果: 稀有掉落 +25%                 │  │
│  │  升级费用: 💰 506                        │  │
│  │                                          │  │
│  │  [      升级 (💰 506)      ]             │  │
│  │                                          │  │
│  └──────────────────────────────────────────┘  │
│                                                │
│  你的金币: 💰 1,234                            │
│                                                │
└────────────────────────────────────────────────┘
```

### 6.2 升级成功弹窗

```
┌────────────────────────────────────┐
│                                    │
│      🎉 升级成功！                  │
│                                    │
│      🎣 鱼竿 Lv.5 → Lv.6           │
│                                    │
│      稀有掉落: +20% → +25%         │
│                                    │
│         [  太棒了！  ]              │
│                                    │
└────────────────────────────────────┘
```

### 6.3 金币不足提示

```
┌────────────────────────────────────┐
│                                    │
│      💰 金币不足                    │
│                                    │
│      需要: 506 金币                 │
│      当前: 234 金币                 │
│      还差: 272 金币                 │
│                                    │
│  [继续钓鱼]     [放置更多鱼]        │
│                                    │
└────────────────────────────────────┘
```

---

## 七、平衡考量

### 7.1 升级节奏分析

| 阶段 | 目标等级 | 累计费用 | 收益率假设 | 预计时长 |
|------|----------|----------|-----------|----------|
| 新手 | Lv.3 | 250 | 5/秒 | 50 秒 |
| 成长 | Lv.5 | 812 | 15/秒 | 55 秒 |
| 中期 | Lv.7 | 2,077 | 30/秒 | 70 秒 |
| 后期 | Lv.10 | 7,486 | 50/秒 | 150 秒 |

**结论**：每次升级间隔约 1-3 分钟，节奏适中

### 7.2 效果感知分析

| 升级 | 稀有度提升 | 可感知性 |
|------|-----------|----------|
| 1→2 | +5% | 弱（需要多次钓鱼体验） |
| 4→5 | +20% 累计 | 中（开始感觉蓝鱼变多） |
| 9→10 | +45% 累计 | 强（紫鱼频繁出现） |

---

## 八、扩展预留

### 8.1 后续升级项（非 MVP）

| 升级项 | 效果 | 优先级 |
|--------|------|--------|
| 展示位扩展 | +1 展示位 | P1 |
| 自动钓鱼 | 无需手动收杆 | P2 |
| 收益加成 | 展示收益 +X% | P2 |
| 钓点解锁 | 开放新钓点 | P2 |

### 8.2 接口预留

```verse
# 通用升级接口
interface IUpgradeable:
    GetCurrentLevel() : int
    GetMaxLevel() : int
    GetUpgradeCost() : int
    CanUpgrade() : logic
    DoUpgrade() : void
    GetEffectDescription() : string
```

---

## 九、UEFN 实现要点

### 9.1 升级管理器

```verse
upgrade_manager := class(creative_device):
    var RodLevel : int = 1
    
    GetUpgradeCost(CurrentLevel : int) : int =
        if CurrentLevel >= 10:
            return -1  # 已满级
        Floor(100.0 * Pow(1.5, CurrentLevel - 1))
    
    TryUpgrade(Player : agent) : logic =
        cost := GetUpgradeCost(RodLevel)
        if cost < 0:
            return false
            
        if PlayerGold[Player] >= cost:
            set PlayerGold[Player] -= cost
            set RodLevel += 1
            NotifyFishingSystem(GetRarityBonus())
            return true
        return false
    
    GetRarityBonus() : float =
        (RodLevel - 1) * 0.05
```

### 9.2 与钓鱼系统联动

```verse
# 钓鱼系统接收升级通知
OnRarityBonusChanged(NewBonus : float) : void =
    set CurrentRarityBonus = NewBonus
    RecalculateLootTable()
```

---

## 十、下一步

**升级系统机制设计完成** ✅

**全部 5 个核心系统机制设计完成：**
- ✅ `@mechanics/fishing.md` - 钓鱼系统
- ✅ `@mechanics/trophy.md` - 战利品系统
- ✅ `@mechanics/base.md` - 基地系统
- ✅ `@mechanics/income.md` - 收益系统
- ✅ `@mechanics/upgrade.md` - 升级系统

下一阶段：调用 `game-economy-designer` 进行整体经济平衡设计

---

*文档生成时间：2025-12-26*
