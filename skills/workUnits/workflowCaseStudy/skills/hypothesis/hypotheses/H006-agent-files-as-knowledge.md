...
> **修正日期**: 2026-01-11 (Run #18)  
> **来源**: workflow-generator 分析 + 9 Agent 文件全量扫描  
> **关联猜想**: refines H003
...
Agent 文件（`.agent.md`）是一种独特的知识沉淀形式，其核心特征是**可执行性**——它封装了复杂任务的执行逻辑，可被系统直接调用，而非仅供人类参考。

### ✅ 已确认论点

1. **Agent 文件是可执行知识沉淀**：与 patterns/ 存储的参考文档不同，Agent 文件可被 `assign-to-agent` 直接调用执行
2. **本质差异是可执行性**：patterns/ 存问题模式（供参考），Agent 文件存解决方案模式（可执行）
3. **知识沉淀有四种形式**：流程型、模板型、规则型、诊断型

### ❌ 已推翻论点

- ~~双模式设计是 Agent 文件的普遍特征~~ → 仅 33% (3/9) 使用双模式，这是特定场景的设计选择

### ⚠️ 待验证论点

- Agent 文件被多工作流共享复用（需检查 `assign-to-agent` 引用）
