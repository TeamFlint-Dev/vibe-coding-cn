...
> **详细说明**: 参见 [R00-SceneGraph-Device-Boundary](../verseResearch/reports/R00-SceneGraph-Device-Boundary/MENTAL-MODEL-MIGRATION.md)

### 快速判定

在设计架构时，首先判定功能是否可 Component化：
...
**满足 3 项以上** → ✅ 可 Component化（优先使用 SceneGraph 架构）

### 架构设计指引

| 判定结果 | 架构方案 | 适用场景 |
|---------|---------|---------|
| **全部可 Component化** | 纯 SceneGraph 架构 | 状态机、生成器、AI系统 |
| **部分可 Component化** | 混合架构（Component + Device） | 钓鱼系统、波次系统 |
| **不可 Component化** | Device主导架构 | UI系统、音效管理 |
...
> **v2.0 关联**: 本规范是 Component化边界原则的具体实现  
> **审计依据**: [architecture-compliance-checklist.md](checklists/architecture-compliance-checklist.md) ARC-004, ARC-006~ARC-010

### Component化边界说明

本节规范定义了 SceneGraph 架构内部的职责分层，是 Component化边界原则的延伸：

- **Component (L3)**: 实现可 Component化的业务逻辑（状态 + 事件 + 流程编排）
- **Manager (L2.5)**: 实现可 Component化的共享服务（有状态但独立于Entity）
- **Helper (L2)**: 提供无状态工具函数（纯逻辑计算）
- **Device**: 处理不可 Component化的能力（输入/UI/音频/资源）

> **关键**: Component/Manager 内的所有逻辑都应是可 Component化的；需要 Device 的能力通过 `@editable` 引用
