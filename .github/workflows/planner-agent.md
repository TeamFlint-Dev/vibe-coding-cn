...
### Step 5: 通知调度器（通过 Issue 事件）

创建一个带有 `pipeline` label 的 Issue，Webhook 服务器会自动接收 `issues` 事件并启动调度：

```bash
# 构建 Issue body，包含调度器需要的所有信息
ISSUE_BODY="## Pipeline Execution Request
...
|-------|----------|--------|
| 1. ingest | \`$INGEST_ID\` | pending |
| 2. classify | \`$CLASSIFY_ID\` | pending |
| 3. extract | \`$EXTRACT_ID\` | pending |
| 4. assemble | \`$ASSEMBLE_ID\` | pending |
| 5. validate | \`$VALIDATE_ID\` | pending |

---
*Auto-generated by Planner Agent*"

# 创建带有 pipeline label 的 Issue
gh issue create \
  --title "Pipeline $PIPELINE_ID - ${{ inputs.pipeline_type }}" \
  --body "$ISSUE_BODY" \
  --label "pipeline"

echo "✅ Pipeline Issue created, webhook will trigger scheduling"
```

> **原理**: 创建带有 `pipeline` label 的 Issue 会触发 `issues` webhook 事件，
> 服务器接收后解析 Issue body 并启动调度器。这样完全通过 GitHub 事件系统通信，
> 不需要直接 HTTP 调用和签名验证。
