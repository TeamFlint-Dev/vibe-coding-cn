...
### Step 5: 通知调度器（通过 repository_dispatch）

使用 GitHub API 发送 `repository_dispatch` 事件，Webhook 服务器会自动接收并启动调度：

```bash
# 通过 GitHub API 发送 repository_dispatch 事件
# 这会触发仓库的 webhook，服务器收到后启动流水线调度
gh api repos/$GITHUB_REPOSITORY/dispatches \
  --method POST \
  -f event_type="pipeline-ready" \
  -f client_payload[pipeline_id]="$PIPELINE_ID" \
  -f client_payload[type]="${{ inputs.pipeline_type }}" \
  -f client_payload[stages]='["ingest", "classify", "extract", "assemble", "validate"]' \
  -f client_payload[stage_ids][ingest]="$INGEST_ID" \
  -f client_payload[stage_ids][classify]="$CLASSIFY_ID" \
  -f client_payload[stage_ids][extract]="$EXTRACT_ID" \
  -f client_payload[stage_ids][assemble]="$ASSEMBLE_ID" \
  -f client_payload[stage_ids][validate]="$VALIDATE_ID" \
  -f client_payload[source_url]="${{ inputs.source_url }}"

echo "✅ repository_dispatch event sent: pipeline-ready"
```

> **原理**: `repository_dispatch` 事件会通过 GitHub Webhook 发送到我们的服务器，
> 服务器接收后解析 `client_payload` 并启动调度器。这样完全通过 GitHub 事件系统通信，
> 不需要直接 HTTP 调用和签名验证。
