...
### ⭐ Phase -1: 猜想库审查（每次执行必做）

**目标**：在开始任何工作前，先审查现有猜想，避免基于错误假设工作

**强制要求**：
1. **阅读 CONJECTURES.md**
   - 查看所有未验证的猜想（状态为 Unverified）
   - 识别与当前任务相关的猜想
   - 评估哪些猜想需要在本次任务中验证
...
### Phase 0: Knowledge Gap Analysis（知识缺口分析）
...
6. **更新猜想库（CONJECTURES.md）** ⭐⭐⭐ 最高优先级
   - **强制要求**：每次任务必须审查和更新
   - **添加新猜想**：记录任何基于推理或单一信源的假设
   - **验证现有猜想**：如果任务验证了某个猜想，立即更新状态
   - **标注置信度**：低/中/高，诚实评估不确定性
   - **避免传播错误**：未验证的猜想不得作为"事实"引用
   - **交叉引用**：在 COMPILATION_LESSONS 中引用相关猜想
...
```
传统模式：用户需求 → 直接实现 → 交付代码
Factory Mode：用户目标 → 审查猜想库 → 知识评估 → 必要调研 → 深度思考 → 实现 → 验证猜想 → 强制沉淀 → 交付代码+知识
```
...
### 知识价值评估（必做）
- [ ] **审查猜想库**：本次任务验证了哪些猜想？（更新 CONJECTURES.md 状态）
- [ ] **添加新猜想**：本次任务产生了哪些新的假设？（记录到 CONJECTURES.md）
...
- [ ] **哪些猜想需要优先验证？** （在 CONJECTURES.md 中标记优先级）
- [ ] **哪些信息源需要补充？** （识别知识盲区）
...
**COMPILATION_LESSONS.json**：
- ✅ 仅记录客观的编译错误和解决方法
- ✅ 忠实描述错误现象、解决步骤
- ❌ 不添加推测性解释或理论假设
- 📝 如有相关猜想，引用 CONJECTURES.md

**CONJECTURES.md**：
- ✅ 记录所有未经多个信息源验证的假设
- ✅ 标注置信度和信息来源
- ✅ 追踪验证状态（Unverified → Verified/Disproven）
- ✅ 只有经过充分验证的猜想才能转为"事实"
...
| 类型 | 定义 | 记录位置 | 标准 |
|------|------|----------|------|
| **已验证事实** | 经过编译验证、多个信息源确认 | ADR, PATTERNS | 可直接引用 |
| **客观错误** | 编译器报告的实际错误 | COMPILATION_LESSONS | 忠实记录，不加推测 |
| **高置信猜想** | 基于官方文档或权威来源的推断 | CONJECTURES (高) | 需注明来源，谨慎使用 |
...
```
开始任务 → 审查 CONJECTURES.md（识别相关假设）
         ↓
做出新假设 → 记录到 CONJECTURES.md（标注置信度）
         ↓
实践验证 → 编译通过/用户反馈/查阅官方文档
         ↓
更新猜想状态 → Verified（转为 ADR/PATTERN）
              │   或 Disproven（标记错误）
         ↓
清理引用 → 修正所有引用该猜想的文档
         ↓
记录教训 → 在 COMPILATION_LESSONS 中仅记录客观错误
         ↓
下次任务 → 循环：审查猜想 → 验证 → 更新
```
...
1. **每次执行先审查猜想库**：Phase -1 强制执行，不得跳过
2. **错误与猜想分离**：COMPILATION_LESSONS 仅记录客观事实
3. **禁止无源头结论**：任何技术结论必须能追溯到信息源或明确标记为猜想
4. **标注置信度**：无法100%确认的信息必须标注为猜想
5. **及时更新**：猜想被验证或证伪后，当次任务内必须更新状态
6. **交叉引用**：ADR 和 LESSONS 必须引用支撑的信息源或相关猜想
7. **定期审查**：每月审查 CONJECTURES.md，清理已解决的项
