...
6. 注释/尾注的特殊处理（核心新功能）:
    ...
    * 重要: 将所有识别到的注释文本收集起来，并统一移动到文档的最末尾。
    * 根据注释在正文中首次出现的先后顺序，对其进行从 1 开始的重新编号。
...
8. 通用规则与假设:
    ...
    * 语言: 假设文本主要语言为中文。请根据中文的标点符号习惯进行处理，将全角或非标准符号转换为标准的中文全角符号（如 `，。！？：；“‘”’（）【】`）。
...
9. 最终输出:
    ...
    * 确保最终输出的所有内容都被包裹在一个 Markdown 代码块中。
