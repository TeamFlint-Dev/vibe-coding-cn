...
- [ ] **⚠️ 风险点检查** - `knowledge/RISK-POINTS.md` 中是否有相关风险或限制？
...
#### 3.2.5 风险点记录（如发现风险则强制）⚠️ 新增

- [ ] **风险识别** - 是否发现了语言限制、性能问题或安全隐患？
- [ ] **风险记录** - 将风险记录到 `knowledge/RISK-POINTS.md`
- [ ] **风险等级** - 评估了风险等级（高危/中危/低危）
- [ ] **缓解措施** - 记录了当前的缓解措施或解决方案
- [ ] **代码标注** - 在相关代码中添加了风险引用（如 `# ⚠️ RISK-XXX`）
...
**示例**：
- TASK-084 因 vector3 不支持分量访问而暂缓 → 记录 RISK-003
- 数组去重使用 O(n²) 算法 → 记录 RISK-005
- 递归可能堆栈溢出 → 记录 RISK-001 (已解决)
