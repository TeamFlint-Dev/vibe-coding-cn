---
name: verse-architecture-selector
...
| 进入架构设计模式 | `design` | 调度 `verse-framework-designer` → 新架构入库 → 返回 |
...
保存当前上下文到 @checkpoint.md
    ↓
调度 verse-framework-designer 进入架构设计模式
    ↓
等待架构设计完成
    ↓
新架构自动入库（🧪实验，引用:0）
...
verse-orchestrator
    ↓ (需求分析后)
verse-architecture-selector ← 本 Skill
    ↓
┌── 匹配成功 → 返回架构上下文 → 继续实现流程
│
├── 需决策 → 等待用户 → 根据选择分流
...
│     └── design → 调度 verse-framework-designer
│
└── 阻断 → 调度 verse-framework-designer
                ↓
...
- [verse-orchestrator](../verse-orchestrator/SKILL.md) - 协调器
- [verse-framework-designer](../verse-framework-designer/SKILL.md) - 框架设计层
