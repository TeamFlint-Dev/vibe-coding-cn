...
`Core/prompts/` 存放本仓库的提示词资产：用 **系统提示词** 约束 AI 的边界与品味，用 **任务提示词** 驱动「需求澄清 → 计划 → 执行 → 复盘」的开发流水线。
...
- 路径约定：`Core/prompts/system_prompts/CLAUDE.md/<版本号>/CLAUDE.md`
- 推荐版本：
...
```bash
# 查看一个任务提示词
sed -n '1,160p' Core/prompts/coding_prompts/标准化流程.md

# 选定系统提示词版本（建议先备份你当前的 CLAUDE.md）
cp Core/prompts/system_prompts/CLAUDE.md/10/CLAUDE.md ./CLAUDE.md
```

## 维护与批量管理（可选）

如果你需要 Excel ↔ Markdown 的批量维护能力，仓库内置了第三方工具：`libs/external/prompts-library/`。建议把它视为"提示词资产的生产工具"，而把 `Core/prompts/` 视为"日常开发的精选集"。
