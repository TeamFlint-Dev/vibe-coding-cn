...
> **类型**: 架构选择器（Selector）  
> **职责**: Component化判定、需求特征提取、架构库匹配、洞察报告生成、变种管理、固化追踪  
...
**调用时机（必须）**:
```
需求分析完成 → 立即调度本 Skill → Component化判定 → 架构匹配 → 决策
```
...
### 0. Component化边界判定（v2.0 新增）

**优先级**: 🔴 最高（在特征提取前执行）

从需求描述中识别功能点，进行 Component化判定：
...
统计结果
    │
    ├─ 全部可 Component化 → 推荐纯 SceneGraph 架构
    ├─ 部分可 Component化 → 推荐混合架构
    └─ 全部不可 Component化 → 推荐 Device 主导架构
```
...
### 功能分解
1. 钓鱼状态机 - ✅✅✅✅✅ (5/5) - 可 Component化
2. 进度计算 - ✅✅✅✅✅ (5/5) - 可 Component化
3. 玩家输入检测 - ❌❌❌❌❌ (0/5) - 不可 Component化（引擎依赖）
4. UI进度条 - ❌❌❌❌❌ (0/5) - 不可 Component化（编辑器预置）
5. 音效播放 - ❌❌❌❌❌ (0/5) - 不可 Component化（资源绑定）

### 判定结论
- **可 Component化**: 2 个功能（40%）
- **不可 Component化**: 3 个功能（60%）
- **推荐架构**: 混合架构（Component 管理逻辑 + Device 提供能力）
